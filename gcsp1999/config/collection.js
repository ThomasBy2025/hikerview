...
let c_path = MY_PARAMS.path || getParam("path", c_Items[0].path);
...
d.push({
    ...
    url: $("#noLoading#").lazyRule((options, c_path, c_title, i) => {
        ...
        return "hiker://empty";
    }, options, c_path, c_info.title, c_index),
    extra: {
...
if (c_data.length == 0) {
    ...
    c_data.map((_, i) => {
        ...
        __.extra.longClick.splice(1, 0, {
            ...
            title: "分享",
            js: $.toString((path, i) => {
                let json = JSON.parse(readFile(path))[i];
